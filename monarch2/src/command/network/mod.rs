...
///
/// This command attempts to select and register the MT on the operator network using the SIM/USIM card installed in the currently selected card slot.<mode > indicates whether the selection is done automatically by the MT or is forced to operator < oper > (whose id is given in format <format>) using a certain access technology, indicated in < AcT>. If the selected operator is not available, no other operator is selected (except if < mode>=4). If the selected access technology is not available, then the same operator is selected using an other access technology. The selected operator name format applies to further read commands (AT+COPS?) also. <mode>=2 forces an attempt to unregister from the network. The selected mode affects to all further network registration (e.g. after <mode>=2, MT is unregistered until <mode>=0 or 1 is selected). This command should be abortable when registration/ de-registration attempt is made.
///
/// CEREG (on page 315) URCs will be received as the module registers/ deregisters from the network.
