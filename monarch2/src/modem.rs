...
use crate::command::{
    ...
    gnss::{
        ...
        responses::GnssAsssitance, types::FixSensitivity, urc::GnssFixReady,
    },
};
use crate::{
    command::{
        ...
        loop {
            match self.urc_subscription.next_message_pure().await {
                ...
                command::Urc::GnssFixReady(fix_ready) => {
                    info!("GNSS Fix Ready received: {:?}", fix_ready);
                ...
                command::Urc::MqttDisconnected(connected) => {
                    info!("MQTT disconnected: {:?}", connected);
                    // self.state.mqtt_connected.signal(connected);
                }
                command::Urc::MqttMessagePublished(published) => todo!(),
                command::Urc::MqttMessageReceived(received) => todo!(),
                command::Urc::MqttSubscribed(subscribed) => todo!(),
                command::Urc::Shutdown(shutdown) => todo!(),
    ...
    pub async fn define_pdp_context(&mut self) -> Result<(), Error> {
        self.client
            .send(&DefinePDPContext {
                ...
                pdp_addr: None,
                d_comp: Some(command::pdp::types::PDPDComp::default()),
                h_comp: Some(command::pdp::types::PDPHComp::default()),
                ipv4_alloc: Some(command::pdp::types::PDPIPv4Alloc::NAS),
                request_type: command::pdp::types::PDPRequestType::NewOrHandover,
                pdp_pcscf_discovery_method: command::pdp::types::PDPPCSCF::Auto,
                for_imcn: false,
                nslpi: false,
                secure_pco: false,
                ipv4_mtu_discovery: false,
                local_addr_ind: false,
                non_ip_mtu_discovery: false,
            })
            .await?;
        Ok(())
    }
...
where
    AtCl: AtatClient,
{
    pub async fn set_gnss_config(&mut self, sensitivity: FixSensitivity) {
        self.client
            .send(&SetGnssConfig {
    ...
    //  updated.
    pub fn check_assistance_data(&mut self, resp: &GnssAsssitance) {
        self.update_almanac = false;
        self.update_ephemeris = false;

        match resp.almanac.available {
            true => {
                info!(
                    "Almanace data is available and should be updated within {}",
                    resp.almanac.time_to_update
                );
                self.update_almanac = resp.almanac.time_to_update <= 0;
            }
            false => {
        ...
        match resp.realtime_ephemeris.available {
            true => {
                info!(
                    "Real-time ephemeris data is available and should be updated within {}",
                    resp.realtime_ephemeris.time_to_update
                );
                self.update_ephemeris = resp.realtime_ephemeris.time_to_update <= 0;
            }
            false => {
    ...
    pub async fn update_gnss_asistance(&mut self) -> Result<(), Error> {
        ...
        let gnss_assist_data = self.client.send(&GetGnssAssitance).await?;

        self.check_assistance_data(&gnss_assist_data);
